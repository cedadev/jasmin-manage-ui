...
export const ProjectCollaboratorsListItem = ({
  project,
  CollaboratorsComponent = "strong",
}) => {
  const currentUser = useCurrentUser();
  ...
  return (
    <ListGroup.Item>
      ...
      {userRole && (
        ...
        <p className="mt-2 mb-0">
          You are {/^[aeiou]/i.test(userRole) ? "an" : "a"}{" "}
...
export const ProjectCreatedAtListItem = ({ project }) => {
  ...
  return (
    <ListGroup.Item>
      Project was created <strong>{createdAt}</strong>.
    </ListGroup.Item>
  );
};
        ...
        <div style={{ display: "inline-flex" }}>
          ...
          {Object.keys(tagkeys).map((d) => (
            <div key={d}>
              ...
              <b>
                <font size="3">{tagkeys[d].data.name}&nbsp;&nbsp;</font>
              </b>
            </div>
          ))}
        </div>
